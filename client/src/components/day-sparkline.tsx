...
export function DaySparkline({ events }: { events: TimelineEvent[] }) {
  ...
  const W        = 600;
  const H        = 96;       // taller to give vertical stacking room
  const PAD_X    = 28;
  const MID_Y    = H / 2;
  const DOT_R    = 5;
  const STACK_GAP = DOT_R * 2 + 3;   // vertical gap between stacked dots
  const BASE_OFFSET = 14;             // distance from midline to first dot in group
  ...
  // ── Group events by (minute-bucket, side) so same-time dots stack ──────────
  // Bucket to nearest minute for grouping purposes
  function bucketKey(iso: string, rating: string): string {
    const d = new Date(iso);
    const min = d.getHours() * 60 + d.getMinutes();
    return `${min}_${rating === "success" ? "top" : "bot"}`;
  }

  // First pass: build group map { bucketKey -> [index, ...] }
  const groupMap = new Map<string, number[]>();
  validEvents.forEach((e, i) => {
    const key = bucketKey(e.ratedAt!, e.rating);
    if (!groupMap.has(key)) groupMap.set(key, []);
    groupMap.get(key)!.push(i);
  });

  // Second pass: for each event, compute its stacked dotY
  const dotPositions: Array<{ x: number; y: number; color: string; event: TimelineEvent }> = validEvents.map((e, i) => {
    const x = timeToX(e.ratedAt!);
    const isSuccess = e.rating === "success";
    const key = bucketKey(e.ratedAt!, e.rating);
    const group = groupMap.get(key)!;
    const posInGroup = group.indexOf(i); // 0-based position in stack
    // Success dots go above midline (decreasing Y), setback go below (increasing Y)
    const dotY = isSuccess
      ? MID_Y - BASE_OFFSET - posInGroup * STACK_GAP
      : MID_Y + BASE_OFFSET + posInGroup * STACK_GAP;

    const customIdx = validEvents
      .filter(ev => !CORE_METRIC_COLORS[ev.metricKey])
      .findIndex(ev => ev.metricKey === e.metricKey);
    const color = getMetricColor(e.metricKey, customIdx < 0 ? 0 : customIdx);

    return { x, y: dotY, color, event: e };
  });
  ...
  return (
    <div className="relative" onClick={e => { if ((e.target as SVGElement).tagName !== "circle") setTooltip(null); }}>
      ...
      >
        ...
        {/* Events — stacked if same time */}
        {dotPositions.map(({ x, y: dotY, color, event: e }, i) => (
          <g key={i}>
            <line x1={x} y1={MID_Y} x2={x} y2={dotY} stroke={color} strokeWidth={1} strokeOpacity={0.35} />
            {/* Invisible hit area */}
            <circle
